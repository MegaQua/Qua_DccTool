...
class JCQ_BS_image_Viewer():
    ...
    def create_labels(self, *args):
        ...
        if matching_items:
            ...
            def selectcurve(label, BSName, item):
                obj_name = f"{BSName}.{item}"
                current_selection = cmds.ls(selection=True)

                if obj_name in current_selection:
                    # 如果对象已经在当前选择中，取消选择该对象
                    cmds.select(obj_name, deselect=True)
                else:
                    # 如果对象不在当前选择中，添加该对象到当前选择
                    cmds.select(obj_name, add=True)

            def selectBS(label, BSName):
                obj_name = BSName
                ...
                if obj_name in current_selection:
                    ...
                    # 如果对象不在当前选择中，添加该对象到当前选择
                    cmds.select(obj_name, add=True)
            def selectBSAltD(label, BSName):
                obj_name = BSName
                # 获取当前的选择
                current_selection = cmds.ls(selection=True)

                # 如果BSName已在当前选择中，则取消选择BSName
                if obj_name in current_selection:
                    cmds.select(obj_name, deselect=True)
                else:
                    # 如果BSName不在当前选择中，则清空当前选择并选择BSName
            ...
            if cmds.objExists(BSName):
                ...
                button = cmds.button(label="selectBS",command=lambda label, BSName=BSName: selectBS(label, BSName))
                button = cmds.button(label="selectBS Alt D",command=lambda label, BSName=BSName: selectBSAltD(label, BSName))
                cmds.text(label="")
                cmds.text(label="")
                cmds.text(label="")
                cmds.text(label="")
            ...
            for item in ShapesNames:
                ...
                if cmds.objExists(BSName + "." + item):
                    button = cmds.button(label=item,command=lambda label, BSName=BSName, item=item: selectcurve(label, BSName, item))
    ...
    def create_Window(self):
        ...
        self.checkBox_selectBS = cmds.checkBox(label='key and select BS', v=False)
