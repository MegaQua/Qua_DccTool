...
class MB_jointPath_tool(QtWidgets.QWidget):
    def __init__(self):
        self.rootJoint = "Shoulder01_R"
        self.childRoot = ["Thumb1_R","index1_R","Middle1_R","Ring1_R","Pinky1_R",]
        self.name = "rig"
        self.conNum = 7
        self.MarkerSize = 200
        self.Active = False
...
for curve_name, obj_names in zip(curve_names, obj_names_list):
    ...
    def main(self):
        alljoints=[]
        mainJoints = [
            "BodyRoot", "Body01", "Body02", "Body03", "Body04",
            "Body05", "Body06", "Body07", "Body08", "Body09",
            "Body10", "Body11", "Body12", "Body13", "Body14",
            "Body15", "Body16", "Body17", "Body18", "Body19", "Body20"]
        for obj in FBSystem().Scene.Components:
            if isinstance(obj, FBModelSkeleton) and obj.Name not in mainJoints:
                alljoints.append(obj)

        #otherjoints = [item for item in alljoints if item not in mainJoints]

        otherjointCons = self.create_dummys(alljoints, dummytype="jointCon", dummylook="HardCross")
        otherjointCons_offset = self.create_dummys(otherjointCons)
        self.jointchain_parent(alljoints, otherjointCons)



        #mainJointsCons = self.create_dummys(mainJoints, dummytype="main", dummylook="HardCross")
        #mainJointsCons_offset = self.create_dummys(mainJointsCons)
        #self.jointchain_parent(mainJoints, mainJointsCons)

        #Cons_location = self.interpolate_between_first_and_last(mainJoints, self.conNum, nonlinear=False)
        #Cons = self.create_dummys(Cons_location, dummytype="con", name=self.name + "_con 1", dummylook="HardCross")
        #Cons_offset = self.create_dummys(Cons)
        #PathCurve = self.create_curve(Cons, "Cons")
        #JointPaths = self.createPathConstrain_s(mainJointsCons_offset, PathCurve)
        #Relation, mainPath = self.pathRolation(mainJointsCons_offset, JointPaths)
        ...
        Cutmainbase = mainbase[:4] + mainbase[5:]
        Cons = self.create_dummys(Cutmainbase, dummytype="Curve_con", dummylook="HardCross")
        Cons_offset = self.create_dummys(Cons)
        PathCurve = self.create_curve(Cons, "Cons")
        ...
        for Chain in jointsChains:
            ...
            Wrist_L=FBFindModelByLabelName("Wrist_R")
            self.jointchain_parent([jointsConsOffsets[0]], [Wrist_L])
