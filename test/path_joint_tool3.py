...
class MB_jointPath_tool(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.childRoot = ["tentacleA1","tentacleB1","tentacleC1","tentacleD1","tentacleE1","tentacleF1"]
        self.name = "rig"
        self.conNum = 10
    ...
    def main(self):
        jointsGP = self.collect_all_joints(self.GPJoint)

        joints_copy = self.create_dummys(jointsGP, dummytype="main")
        ...
        for jointsChain in jointsChains:
            ...
            self.jointchain_parent(jointsConsOffsets, joints_copy)

            for i in range(1, len(jointsConsOffsets)):
                jointsConsOffsets[i].Parent = jointsCons[i - 1]

            #self.Transformation_lock(jointsConsOffsets)
