...
class MyWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        #button4.clicked.connect(self.bake)

        self.checkbox = QtWidgets.QCheckBox("Separate shake Animation", self)
        self.checkbox.setChecked(True)  # 默认勾选

        #button5 = QtWidgets.QPushButton("重构shake animation", self)
        # button5.clicked.connect(self.你的方法名)  # 连接到相应的方法
        ...
        #layout.addWidget(button4, 3, 1)  # 第3行，第0列


        self.setLayout(layout)
        #self.resize(400, 300)  # 设置窗口的默认尺寸为宽度400和高度300
        self.setWindowFlags(self.windowFlags() | QtCore.Qt.WindowStaysOnTopHint)  # 设置窗口置顶    def conbinetool(self):
    ...
    def create_smooth_and_noise_groups(self,selected_obj):
        ...
        smooth_group = cmds.group(em=True, name=selected_obj + '_smooth_anim_GP')
        noise_group=cmds.group(em=True, name=selected_obj + '_shake_anim_GP')
    ...
    def constraint_cam(self):
        ...
        # 复制相机并重命名
        aim_camera = cmds.group(em=True, name=self.Object + "_aim_animation")
        ...
        #print(cmds.getAttr(f"{camera_name}.focusDistance"))
        ...
        cmds.setAttr(f"{aim_camera}.rotateAxis", *new_rotate_axis, type="double3")
        # 创建瞄准约束
