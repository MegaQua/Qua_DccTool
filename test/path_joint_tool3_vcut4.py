...
class MB_jointPath_tool(QtWidgets.QWidget):
    def __init__(self):
        self.GPJoint = "tentacleCenter1"
        self.childRoot = ["tentacleABa_14","tentacleBBa_14","tentacleCBa_14","tentacleDBa_14","tentacleEBa_14","tentacleFBa_14"]
        self.LineRoot = "tentacleCuvre1"
        self.name = "rig"
        self.conNum = 10
...
for curve_name, obj_names in zip(curve_names, obj_names_list):
    ...
    def main(self):
        ...
        curve_base = self.collect_all_joints(self.LineRoot)
        joints_base.append(self.createEndNull(curve_base))
        curve_base.append(self.createEndNull(curve_base))
        joints_base[-1].Parent = joints_base[-2]
        curve_base[-1].Parent = curve_base[-2]

        FKbase = joints_base.copy()
        FKbase.insert(15, FKbase[15])

        FKCons = self.create_dummys(FKbase, dummytype="FKcon", dummylook="HardCross")
        FKCons_offset = self.create_dummys(FKCons)
        for i in range(1, len(FKCons_offset)):
            FKCons_offset[i].Parent = FKCons[i - 1]
        self.jointchain_parent(joints_base, FKCons[:15]+FKCons[16:])

        Cons = self.create_dummys(curve_base, dummytype="con", name=self.name + "_con 1", dummylook="HardCross")
        Cons_offset = self.create_dummys(Cons)
        ...
        Y_Cons = self.create_dummys(curve_base, dummytype="Y", offset=(0, 1, 0))
        for i in range(len(Cons)):
            Y_Cons[i].Parent = Cons[i]
        Y_PathCurve = self.create_curve(Y_Cons, "Cons_Y")
        curve_names.append(Y_PathCurve.Name)
        obj_names_list.append([obj.Name for obj in Y_Cons if obj is not None])
        joints_copy_yoffset = self.create_dummys(FKbase, dummytype="Y", offset=(0, 1, 0), P=True)

        Z_Cons = self.create_dummys(curve_base, dummytype="Z", offset=(0, 0, 1))
        for i in range(len(Cons)):
            Z_Cons[i].Parent = Cons[i]
        Z_PathCurve = self.create_curve(Z_Cons, "Cons_Z")
        curve_names.append(Z_PathCurve.Name)
        obj_names_list.append([obj.Name for obj in Z_Cons if obj is not None])
        joints_copy_zoffset = self.create_dummys(FKbase, dummytype="Z", offset=(0, 0, 1),P=True)
        ...
        for jointsChain in jointsChains:
            ...
            self.jointchain_parent(jointsConsOffsets[:3],FKCons[13:16])
            self.jointchain_parent(jointsConsOffsets[-3:],FKCons[16:19])
