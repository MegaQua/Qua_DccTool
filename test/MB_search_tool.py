...
def process_offset_objects(search_keyword, action_keyword=None):
    ...
    for obj in offset_objects:
        if action_keyword is None:
            ...
            Transformation_lock([obj])
        else:
            print(f"Unknown action keyword: {action_keyword}")


# 调用函数，搜索以 '_offset' 结尾的对象，并根据操作关键字进行操作
search_keyword = '_offset'
action_keyword = "Lock"  # 或者 'Lock'

process_offset_objects(search_keyword, action_keyword)
