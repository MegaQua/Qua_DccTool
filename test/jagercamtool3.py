...
class MyWindow(QtWidgets.QWidget):
    ...
    def create_smooth_and_noise_groups(self,selected_obj):
        ...
        smooth_group = cmds.duplicate(selected_obj, name=selected_obj + '_smooth_anim')[0]
        cmds.setAttr(f'{smooth_group}.translate', 0, 0, 0)
        cmds.setAttr(f'{smooth_group}.rotate', 0, 0, 0)
        #cmds.setAttr(f"{smooth_group}.rotateAxis", 0, 0, 0)
    ...
    def constraint_cam(self):
        ...
        # 复制相机并重命名
        aim_camera = cmds.duplicate(camera_name, name=camera_name + "_aim_anim")[0]
        cmds.setAttr(f'{aim_camera}.translate', 0, 0, 0)
        cmds.setAttr(f'{aim_camera}.rotate', 0, 0, 0)

        #offset_values = cmds.getAttr(f"{camera_name}.rotateAxis")[0]
        cmds.setAttr(f"{aim_camera}.rotateAxis", 0,0,0)
        ...
        if self.checkbox.isChecked():
            ...
            """
            camera_name=self.Smooth_Object
            
            # 创建一个新的locator作为瞄准目标
            aim_locator_Smooth = cmds.spaceLocator(name=self.Object + "_aim_locator_Smooth")[0]

            # 将locator放置在原始相机的位置
            cmds.matchTransform(aim_locator_Smooth, self.Object, position=True)
            cmds.matchTransform(aim_locator_Smooth, self.Object, rotation=True)
            cmds.rotate(-offset_values[0], -offset_values[1], -offset_values[2], aim_locator_Smooth, relative=True, objectSpace=True)
            cmds.move(0, 0, cmds.getAttr(f"{self.Object}.focusDistance"), aim_locator_Smooth, relative=True, objectSpace=True, worldSpaceDistance=0)
            cmds.parentConstraint(camera_name, aim_locator_Smooth, maintainOffset=True)

            # 创建瞄准约束
            cmds.aimConstraint(aim_locator_Smooth, camera_name, aimVector=[0, 0, -1], upVector=[0, 1, 0], worldUpType="vector")
            ...
            for attr in ["translateX", "translateY", "translateZ"]:
                ...
                if cmds.listConnections(camera_name + '.' + attr, type='animCurve'):
                    cmds.copyKey(camera_name, attribute=attr)
                    cmds.pasteKey(smooth_camera_name, attribute=attr)
            """
            # 复制相机并重命名
            new_camera_name = self.Smooth_Object
            aim_camera = cmds.duplicate(new_camera_name, name=new_camera_name + "_Smooth_aim_anim")[0]
            cmds.setAttr(f'{aim_camera}.translate', 0, 0, 0)
            cmds.setAttr(f'{aim_camera}.rotate', 0, 0, 0)
            ...
            # 创建一个新的locator作为瞄准目标
            _Smooth_aim_locator = cmds.spaceLocator(name=self.Object + "_Smooth_aim_locator")[0]

            # 将locator放置在原始相机的位置
            cmds.matchTransform(_Smooth_aim_locator, self.Object, position=True)
            cmds.matchTransform(_Smooth_aim_locator, self.Object, rotation=True)
            cmds.rotate(-offset_values[0], -offset_values[1], -offset_values[2], _Smooth_aim_locator, relative=True, objectSpace=True)
            cmds.move(0, 0, cmds.getAttr(f"{self.Object}.focusDistance"), _Smooth_aim_locator, relative=True, objectSpace=True, worldSpaceDistance=0)
            cmds.parentConstraint(new_camera_name, _Smooth_aim_locator, maintainOffset=True)

            # 创建瞄准约束
            cmds.aimConstraint(_Smooth_aim_locator, aim_camera, aimVector=[0, 0, -1], upVector=[0, 1, 0], worldUpType="vector")

            # 复制原始相机的动画到新相机
            for attr in ["translateX", "translateY", "translateZ"]:
                # 如果原相机上有动画曲线，则复制到新相机
                if cmds.listConnections(new_camera_name + '.' + attr, type='animCurve'):
                    cmds.copyKey(new_camera_name, attribute=attr)
                    cmds.pasteKey(aim_camera, attribute=attr)


        # 创建约束
        cmds.pointConstraint(aim_camera, f"{namespace}:camMainAndRotateY", maintainOffset=False)
        cmds.orientConstraint(aim_camera, f"{namespace}:camRotateZ ", skip=("x", "y"), maintainOffset=False)
        cmds.aimConstraint(aim_locator, f"{namespace}:camRotateX", aimVector=[-0, 0, -1], upVector=[0, 1, 0], worldUpType="vector", skip=("y", "z"))
        cmds.aimConstraint(aim_locator, f"{namespace}:camMainAndRotateY",  aimVector=[0, 0, -1], upVector=[0, 1, 0], worldUpType="vector", skip=("x", "z"))
        #cmds.pointConstraint(_Smooth_aim_locator, f"{namespace}:camAim", maintainOffset=False)
        cmds.connectAttr(f"{camera_name}.focalLength", f"{namespace}:camMainOpt.focalLength",)
        cmds.connectAttr(f"{camera_name}.focusDistance", f"{namespace}:focusPoint.translateZ", )

        if self.Smooth_Object :
            if self.checkbox2.isChecked():
                cmds.parentConstraint(camera_name, f"{namespace}:camShakeAnim", maintainOffset=True, skipRotate="z")
            else:
                cmds.parentConstraint(camera_name, f"{namespace}:camShakeAnim", maintainOffset=True)
