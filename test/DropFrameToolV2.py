...
class DropFrameTool(QtWidgets.QWidget):
    ...
    def record_keyframes(self, obj_list, start_frame, end_frame, n):
        ...
        if self.translate_check.isChecked():
            attributes_to_check.extend(["translateX", "translateY", "translateZ"])
        if self.rotate_check.isChecked():
            attributes_to_check.extend(["rotateX", "rotateY", "rotateZ"])
        if self.scale_check.isChecked():
            attributes_to_check.extend(["scaleX", "scaleY", "scaleZ"])
        ...
        while current_frame <= end_frame:
            record_frame_data(current_frame)
            current_frame += n

        # 确保记录最后一帧
        if current_frame - n < end_frame:
            record_frame_data(end_frame)
    ...
    def apply_keyframes(self, obj_list, key_frames_dict, start_frame, end_frame, n, layer_name):
        ...
        for obj_name in obj_list:
            ...
            for kf in key_frames:
                ...
                # 确定帧范围
                if frame_start < start_frame or frame_start >= end_frame:
                    continue
                frame_range = range(frame_start, min(frame_end, end_frame + 1))

                for attr, value in kf.items():
                    if attr == 'frame':  # 跳过关键帧信息字段
                        continue

                    # 检查属性是否锁定
                    attr_path = f"{obj_name}.{attr}"
                    if cmds.getAttr(attr_path, lock=True):
                        cmds.warning(f"Skipping locked attribute: {attr_path}")
                        continue
