...
class DropFrameTool(QtWidgets.QWidget):
    ...
    def remove_nokeyframe_obj(self,selected_set,start_frame,end_frame):
        ...
        return filtered_set
    """
    def remove_connected_obj(self, selected_set, start_frame, end_frame):

        #移除在勾选属性范围内存在连接的对象，只保留没有任何连接的对象。
        ...
        for obj in selected_set:
            ...
            if self.translate_check.isChecked():
                for attr in ["translateX", "translateY", "translateZ"]:
                    if cmds.listConnections(f"{obj}.{attr}", s=True, d=False):
                        has_connection = True
                        break

            # 检查旋转
            if not has_connection and self.rotate_check.isChecked():
                for attr in ["rotateX", "rotateY", "rotateZ"]:
                    if cmds.listConnections(f"{obj}.{attr}", s=True, d=False):
                        has_connection = True
                        break
            ...
            # 检查缩放
            if not has_connection and self.scale_check.isChecked():
                for attr in ["scaleX", "scaleY", "scaleZ"]:
                    if cmds.listConnections(f"{obj}.{attr}", s=True, d=False):
                        has_connection = True
                        break
