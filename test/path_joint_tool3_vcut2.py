...
class MB_jointPath_tool(QtWidgets.QWidget):
    ...
    def main(self):
        ...
        joints_base_copy = self.create_dummys(joints_base, dummytype="main")
        ...
        for jointsChain in jointsChains:
            jointsCons = self.create_dummys(jointsChain, dummytype="con", dummylook="HardCross")
            ...
            self.jointchain_parent(jointsConsOffsets[:8]+ jointsConsOffsets[9:], joints_base_copy)
            self.jointchain_parent([jointsConsOffsets[8]], [joints_base_copy[8]] )



        joints_base_copy.append(self.createEndNull(joints_base_copy))

        for i in range(len(joints_base_copy) - 1):
            joints_base_copy[i + 1].Parent = joints_base_copy[i]

        #Cons_location = self.interpolate_between_first_and_last(joints_base_copy, self.conNum,nonlinear=False)
        #Cons = self.create_dummys(Cons_location, dummytype="copy", name=self.name + "_con 1", dummylook="HardCross")
        Cons = self.create_dummys(joints_base_copy, dummytype="con", name=self.name + "_con 1", dummylook="HardCross")
        Cons_offset = self.create_dummys(Cons)
        ...
        Y_PathCurve = self.create_curve(Y_Cons, "Cons_Y")
        joints_copy_yoffset = self.create_dummys(joints_base_copy, dummytype="Y", offset=(0, 1, 0), P=True)

        Z_Cons = self.create_dummys(Cons, dummytype="Z", offset=(0, 0, 1))
        for i in range(len(Cons)):
            Z_Cons[i].Parent = Cons[i]
        Z_PathCurve = self.create_curve(Z_Cons, "Cons_Z")
        joints_copy_zoffset = self.create_dummys(joints_base_copy, dummytype="Z", offset=(0, 0, 1),P=True)


        JointPaths = self.createPathConstrain_s(joints_base_copy, PathCurve)
        Y_JointPaths = self.createPathConstrain_s(joints_copy_yoffset, Y_PathCurve)
        Z_JointPaths = self.createPathConstrain_s(joints_copy_zoffset, Z_PathCurve)
        Relation, mainPath = self.pathRolation(joints_base_copy, JointPaths)
        _, Y_OffsetmainPath = self.pathRolation(joints_copy_yoffset, Y_JointPaths)
        _, Z_OffsetmainPath = self.pathRolation(joints_copy_yoffset, Z_JointPaths)
        self.warppathtogether(Relation, mainPath, Y_OffsetmainPath)
        self.warppathtogether(Relation, mainPath, Z_OffsetmainPath)

        self.createChainAim(joints_base_copy, joints_copy_yoffset)
        self.createChainScale(joints_base_copy, joints_copy_yoffset,joints_copy_zoffset)
