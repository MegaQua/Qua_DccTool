...
class MR_Window(QtWidgets.QWidget):
    ...
    def exportBSbyJson(self):
        # get data from scene
        BSname = self.sceneBS_list.currentText()
        startFrames = self.start_frame_text.text()
        endFrames = self.end_frame_text.text()
        folder_path = self.export_path_text.text()
        filename = self.filename_text.text()
        ...
        # create json
        pyjson = {}
        pyjson['numPoses'] = len(bslist)
        pyjson['numFrames'] = numFrames
        pyjson['facsNames'] = bslist
        pyjson['weightMat'] = weightMat
        print(pyjson)
