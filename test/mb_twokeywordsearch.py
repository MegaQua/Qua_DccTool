...
def process_offset_objects(start_keyword, end_keyword, action_keyword=None):
    ...
    for obj in scene.Components:
        ...
        if isinstance(obj, (FBVideoIn, FBVideoOut, FBAudioOut, FBAudioIn)):
            continue
        if obj.Name.startswith(start_keyword) and obj.Name.endswith(end_keyword):
    ...
    if offset_objects:
        ...
        print("No objects found with the specified search keywords.")

# 调用函数，搜索以 'tentacleA' 开头并以 '_curveCon' 结尾的对象，并根据操作关键字进行操作
start_keyword = 'tentacleA'
end_keyword = '_curveCon'

process_offset_objects(start_keyword, end_keyword)
