...
class MB_jointPath_tool(QtWidgets.QWidget):
    def __init__(self):
        self.rootJoint = "tentacleF_1"
        self.childRoot = "tentacleF_1"
        self.name = "rigF"
        self.conNum = 10
    ...
    def main(self):
        ...
        jointCons = self.create_dummys(joints, dummytype="Joint_con", dummylook="HardCross")
        joint_offsets = self.create_dummys(jointCons)
        for i in range(len(joint_offsets) - 1):
            joint_offsets[i + 1].Parent = jointCons[i]
        self.jointchain_parent(joints,jointCons)

        joints_copy = self.create_dummys(joints, dummytype="main")

        self.jointchain_parent(joint_offsets,joints_copy)
        ...
        Cons = self.create_dummys(joints_copy, dummytype="Curve_con", dummylook="HardCross")
        #Cons = self.create_dummys(Cons_location, dummytype="copy", name=self.name + "_con 1", dummylook="HardCross")
        Cons_offset = self.create_dummys(Cons)
