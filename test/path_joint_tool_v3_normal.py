...
for curve_name, obj_names in zip(curve_names, obj_names_list):
    ...
    def main(self):
        ...
        Y_PathCurve = self.create_curve(Y_Cons, "Cons_Y")
        joints_copy_yoffset = self.create_dummys(mainbase, dummytype="Y", offset=(0, 1, 0), P=True)
        ...
        for Chain in jointsChains:
            jointsChain = self.create_dummys(Chain, dummytype="main")
            for i in range(1, len(jointsChain)):
                jointsChain[i].Parent = jointsChain[i - 1]
            self.jointchain_parent(Chain, jointsChain)
        ...
        for i in offsetlist:
            self.jointchain_parent(i[:1], mainCon[-2:], num=1)
