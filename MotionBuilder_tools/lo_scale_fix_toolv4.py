...
class Fix_tool(QtWidgets.QWidget):
    ...
    def classify_null_objects_by_relation(self, relation_long_name):
        ...
        elif "itm0040" in namespace:
            # 匹配 tentacleCenter[1-2位数字]_Y, tentacleCenter[1-2位数字]_Z 和 tentacleCenter[1-2位数字]_FKcon_offset
            y_pattern = re.compile(f"^{namespace}:Arm\d{{1,2}}_main_offset_X", re.IGNORECASE)
            z_pattern = re.compile(f"^{namespace}:Arm\d{{1,2}}_main_offset_Z", re.IGNORECASE)
            fkcon_offset_pattern = re.compile(f"^{namespace}:Arm\d{{1,2}}_main_offset", re.IGNORECASE)

        elif "itm0041" in namespace:
            # 匹配 tentacleCenter[1-2位数字]_Y, tentacleCenter[1-2位数字]_Z 和 tentacleCenter[1-2位数字]_FKcon_offset
            y_pattern = re.compile(f"^{namespace}:tentacle_\\d{{1,2}}_main_offset_X", re.IGNORECASE)
            z_pattern = re.compile(f"^{namespace}:tentacle_\\d{{1,2}}_main_offset_Z", re.IGNORECASE)
            fkcon_offset_pattern = re.compile(f"^{namespace}:tentacle_\\d{{1,2}}_main_offset", re.IGNORECASE)
    ...
    def check_inherit_type_all_set_to_one(self, namespace):
        """
        检查指定命名空间内的所有 joint 是否已经将 InheritType 设置为 1。
        如果全部为 1，则弹出对话框提示无需处理。

        参数:
        namespace (str): 要检查的命名空间。

        返回:
        bool: 如果所有 joint 的 InheritType 属性都为 1，则返回 True；否则返回 False。
        """
        all_set_to_one = True
        for obj in FBSystem().Scene.Components:
            if isinstance(obj, FBModelSkeleton) and obj.LongName.startswith(f"{namespace}:"):
                for prop in obj.PropertyList:
                    if prop.Name == "InheritType":
                        if prop.Data != 1:
                            all_set_to_one = False
                            break
                if not all_set_to_one:
                    break

        if all_set_to_one:
            # 弹出日语提示对话框
            QtWidgets.QMessageBox.information(self, "処理不要", f"{namespace} の全ての joint は InheritType が 1 に設定されています。操作は不要です。")
            return True
        return False
    ...
    def main(self):
        ...
        # 标志变量，检查所有 namespace 中的 InheritType 是否全部为 1
        all_namespaces_set_to_one = True

        # 遍历每个 namespace，检查所有 joint 的 InheritType 是否都已设置为 1
        for namespace in namespaces:
            if not self.check_inherit_type_all_set_to_one(namespace):
                all_namespaces_set_to_one = False  # 如果有一个 namespace 不满足条件，则标志设为 False

        # 如果所有 namespace 的 joint 的 InheritType 都为 1，则中止后续操作
        if all_namespaces_set_to_one:
